...
{
    ingame = 0;
    pS_F->setIsVisible(false);
    pS_FShadow->setIsVisible(false);
...
ActionLayer::ActionLayer(int GameMode, std::string *RedName, std::string *GreenName, std::string *BlueName, std::string *YellowName)
{
	...
	{
		case GAMERS_3:	
			tokens[0] = new SpriteTokenClass(this,CCSprite::spriteWithFile(tokens_token_red),CCSprite::spriteWithFile(tokens_token_shadow),0,ccp(R_X,R_Y)); tokens[0]->pS_F->setTag(ID_RED); tokens[0]->m_groupname=RedName;
			tokens[1] = new SpriteTokenClass(this,CCSprite::spriteWithFile(tokens_token_green),CCSprite::spriteWithFile(tokens_token_shadow),0,ccp(G_X,G_Y)); tokens[1]->pS_F->setTag(ID_GREEN);	tokens[1]->m_groupname=GreenName;
			tokens[2] = new SpriteTokenClass(this,CCSprite::spriteWithFile(tokens_token_blue),CCSprite::spriteWithFile(tokens_token_shadow),0,ccp(B_X,B_Y)); tokens[2]->pS_F->setTag(ID_BLUE); tokens[2]->m_groupname=BlueName;			
		...
		case GROUP_4:	
			...
			tokens[1] = new SpriteTokenClass(this,CCSprite::spriteWithFile(tokens_token_green),CCSprite::spriteWithFile(tokens_token_shadow),0,ccp(G_X,G_Y)); tokens[1]->pS_F->setTag(ID_GREEN);	tokens[1]->m_groupname=GreenName;
			tokens[2] = new SpriteTokenClass(this,CCSprite::spriteWithFile(tokens_token_blue),CCSprite::spriteWithFile(tokens_token_shadow),0,ccp(B_X,B_Y)); tokens[2]->pS_F->setTag(ID_BLUE); tokens[2]->m_groupname=BlueName;
			tokens[3] = new SpriteTokenClass(this,CCSprite::spriteWithFile(tokens_token_yellow),CCSprite::spriteWithFile(tokens_token_shadow),0,ccp(Y_X,Y_Y)); tokens[3]->pS_F->setTag(ID_YELLOW); tokens[3]->m_groupname=YellowName;
			groups[0].CurGroupPosition(0);	groups[0].NextGroupPtr(&groups[1]);
			groups[1].CurGroupPosition(0);	groups[1].NextGroupPtr(&groups[2]);
    ...
    char actions[45]={'O','R','P','O','O','P','O','R','P','O',
                        'R','O','R','P','P','R','O','P','P','P',
                        'R','R','R','P','R','O','P','O','P','O',
                        'R','R','R','P','P','O','P','R','R','P',
                        'R','O','O','P','O'};
...
//8
	...
	//GameMenu
	CCMenuItemImage* itm_rewind = CCMenuItemImage::itemFromNormalImage(GameMenu_menu_rewind,GameMenu_menu_rewind_a,this,menu_selector(ActionLayer::itm_rewindCallback));
	...
	//Answers
	itm_yes = CCMenuItemImage::itemFromNormalImage(RightControl_itm_yes_norm,RightControl_itm_yes_norm,this,menu_selector(ActionLayer::itm_yesCallback));
	itm_no = CCMenuItemImage::itemFromNormalImage(RightControl_itm_no_norm,RightControl_itm_no_norm,this,menu_selector(ActionLayer::itm_noCallback));
	itm_other = CCMenuItemImage::itemFromNormalImage(RightControl_itm_other_norm,RightControl_itm_other_norm,this,menu_selector(ActionLayer::itm_otherCallback));
	...
	pL_Timer->setPosition(ccp(-120,y)); //-110
		//timer
	CCSprite* pS_TimerDesk = CCSprite::spriteWithFile(Timer_timer_forest); 
	pL_Timer->addChild(pS_TimerDesk,1);
		//timer_digits
	CCSprite* pS_Timer_digits = CCSprite::spriteWithFile(Timer_timer_digits_passive);
	pL_Timer->addChild(pS_Timer_digits,1);
	pS_Timer_digits->setPosition(ccp(11,-3));
		//timer_digits_light
	CCSprite* pS_Timer_digits_l = CCSprite::spriteWithFile(Timer_timer_digits_light);
	pS_Timer_digits_l->setOpacity(255);
	pL_Timer->addChild(pS_Timer_digits_l,3,ID_DIGITS_L);
	pS_Timer_digits_l->setPosition(ccp(-4,-24));
		//button
	itm_Timer_start = CCMenuItemImage::itemFromNormalImage(Timer_itm_timer_norm,Timer_itm_timer_sel,this,menu_selector(ActionLayer::itm_TimerStartCallback));
	itm_Timer_pause = CCMenuItemImage::itemFromNormalImage(Timer_itm_timer_stop_norm,Timer_itm_timer_stop_sel,this,menu_selector(ActionLayer::itm_TimerPauseCallback));
	itm_Timer_run = CCMenuItemImage::itemFromNormalImage(Timer_itm_timer_run_norm,Timer_itm_timer_run_sel,this,menu_selector(ActionLayer::itm_TimerRunCallback));
	...
	itm_Timer_run->setIsVisible(false);
		//button_light
	CCSprite* pS_Timer_button_l = CCSprite::spriteWithFile(Timer_itm_timer_norm_light);
...
{	/*
    ...
    */
    ...
    const char* hints_actions[3] = { "объясните друзьям ","покажите как выглядит ","нарисуйте на бумаге "};
    const char* hints_actions_2[3] = { "объясняли ","паказывали ","рисовали "};
    switch (gamestate) {
        case 0:
        ...
        case 1:
            ...
            strcat(text_hint,hints_actions[hint_act]);
        ...
        case 2:
            ...
            strcat(text_hint,hints_actions_2[hint_act]);
...
void ActionLayer::itm_otherCallback(CCObject* pSender)
{
	...
	case GAMERS_3:
		switch(active_token->m_CurNumber)
		{
			case 0:
				//Who answer
				itm_1opponent = CCMenuItemImage::itemFromNormalImage(RightControl_c_blank1,RightControl_c_blank1,this,menu_selector(ActionLayer::itm_1opponentCallback));
				itm_1opponent->setTag(1);
				itm_2opponent = CCMenuItemImage::itemFromNormalImage(RightControl_c_blank2,RightControl_c_blank2,this,menu_selector(ActionLayer::itm_2opponentCallback));			
				itm_2opponent->setTag(2);
				mn_WhoAnswer = CCMenu::menuWithItems(itm_1opponent, itm_2opponent, NULL);				
				break;
			case 1:
				//Who answer
				itm_1opponent = CCMenuItemImage::itemFromNormalImage(RightControl_c_blank0,RightControl_c_blank0,this,menu_selector(ActionLayer::itm_1opponentCallback));
			...
			case 2:
				...
				itm_1opponent = CCMenuItemImage::itemFromNormalImage(RightControl_c_blank0,RightControl_c_blank0,this,menu_selector(ActionLayer::itm_1opponentCallback));
				itm_1opponent->setTag(0);
				itm_2opponent = CCMenuItemImage::itemFromNormalImage(RightControl_c_blank1,RightControl_c_blank1,this,menu_selector(ActionLayer::itm_2opponentCallback));			
	...
	case GROUP_4:	
		switch(active_token->m_CurNumber)
		{
			case 0:
				...
				itm_2opponent = CCMenuItemImage::itemFromNormalImage(RightControl_c_blank2,RightControl_c_blank2,this,menu_selector(ActionLayer::itm_2opponentCallback));
				itm_2opponent->setTag(2);
				itm_3opponent = CCMenuItemImage::itemFromNormalImage(RightControl_c_blank3,RightControl_c_blank3,this,menu_selector(ActionLayer::itm_3opponentCallback));
			...
			case 3:
				...
				itm_2opponent = CCMenuItemImage::itemFromNormalImage(RightControl_c_blank1,RightControl_c_blank1,this,menu_selector(ActionLayer::itm_2opponentCallback));
				itm_2opponent->setTag(1);
				itm_3opponent = CCMenuItemImage::itemFromNormalImage(RightControl_c_blank2,RightControl_c_blank2,this,menu_selector(ActionLayer::itm_3opponentCallback));
...
void ActionLayer::itm_card4Callback(CCObject* pSender)
{
	curCardPrice = 4;
	mn_Cards->setIsTouchEnabled(0);
	checked_card = CCSprite::spriteWithFile(RightControl_c4);
	addChild(checked_card,2,CARD_LAYER);
...
void ActionLayer::RewindAnimation(CCLayer* parent)
{
	...
	for(int i=0;i<4;i++)
	{
		CCSprite* sprite_array[] = {CCSprite::spriteWithFile(RewindAnimation_rewind),CCSprite::spriteWithFile(RewindAnimation_rewind2)};
		int numspr = rand()%2;
...
void ActionLayer::RestoreMenuState()
{
    ...
    if (menustate&16)
    {
        mn_WhoAnswer->setIsTouchEnabled(true);
    }
}
...
{    
    ...
    for(int j=0;j<label->getChildrenCount();j++)
	{
		...
		chars[j]->runAction(CCRepeatForever::actionWithAction((CCActionInterval*)
			...
			CCSequence::actions(CCTintTo::actionWithDuration(0.5,230,230,230),CCTintTo::actionWithDuration(0.5,clr.b,clr.g,clr.r),NULL)->copy()->autorelease(),
	...
	*/
	...
	CCSprite* pS_buttonHelp = CCSprite::spriteWithFile(GameMenu_help);
...
WinnerMenu::WinnerMenu(ActionLayer* al, int Group)
{
	...
	//desk
	CCSprite* pS_Winners = CCSprite::spriteWithFile("winners/desk.png");
    ...
    CCMenuItemImage* itm_mainmenu = CCMenuItemImage::itemFromNormalImage("winners/btn_mainmenu.png", "winners/btn_mainmenu_a.png", this,menu_selector(WinnerMenu::itm_mainmenuCallback));
    if(al->allactivetokens==1) itm_continue->setIsVisible(false);
    CCMenu* p_Menu = CCMenu::menuWithItems(itm_continue,itm_newgame,itm_mainmenu,NULL);
