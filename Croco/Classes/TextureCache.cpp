...
, m_nNumberOfLoadedSprites(0)
{
    ...
    CCSprite* pS_background = CCSprite::spriteWithFile("MainMenu/preload.png");
    addChild(pS_background);
    pS_background->setPosition(CCPointMake(size.width/2, size.height/2));
    
    m_pLabelLoading = CCLabelTTF::labelWithString("Загрузка..", "fonts/neucha.ttf", 30);
	m_pLabelPercent = CCLabelTTF::labelWithString("%0", "fonts/neucha.ttf", 30);
